...
baseline.files <-
   list.files(
      path = here("data/OOHR2000/baseline"),
      pattern = "*.txt",
      full.names = TRUE)
lightsource <-
   OO2df(
...
cuvettes <- OO2df(datafile = cuvette.files[1], version = "1")
...
<<'MB-abs-coeff'>>=
abs.coeff <- LoadRData2Variable(here("data/abscoeff.rda"))
...
<<'figref-MB-abs-coefficient', eval=FALSE, echo=FALSE>>=
slice <- 
   abs.coeff %>%
   mutate(solution = paste0("\\ch{", sub("\\+", " + ", solution), "}")) %>%
   filter(dye == "MB") %>%
...
ggplot(
   ...
   annotate(
      "text", x = 405, y = 4.9e4, 
      label = 
         "\\footnotesize\\scalebox{0.60}{\\chemfig{[7]H_3C-N(-[6]CH_3)-[:30]*6(=-(*6(=\\chembelow{S}{\\scriptscriptstyle\\oplus}(!\\nobond\\chemabove{Cl}{\\scriptscriptstyle\\ominus})-(*6(-=(-N(-[1]CH_3)-[6]CH_3)-=-=))--N=-))--=-)}}") +
   geom_label_repel(
      data = slice,
      aes(x = wavelength, 
          y = k,
...
mb.bands.fitted <- 
   rbind(
      ...
      cbind(
         solvent = "H2O+EtOH",
         read.table(
            here("data/abscoeff-H2O+EtOH.peaks"), 
            skip = 1,
            # skip the last three columns (fityk's output is verbose)
            colClasses = c(rep(NA, 6), rep("NULL", 3)),
            col.names = c("peak", "type", "center", "height", "area", "fwhm", "", "", ""))))
...
<<'figref-MB-water-fitted', eval=FALSE, echo=FALSE>>=
df <- read.table(file = here("data/abscoeff-H2O.dat"))
names(df) <- 
   c("wl", "abs", 
...
ggplot() + 
   ...
   geom_text(
      ...
      size = 2.0) +
   # solvent label
   annotate("text", x = 500, y = 54E3, label = "\\ch{H2O}", hjust = 0, vjust = 1, size = 2) +
   coord_cartesian(xlim = c(500, 750), ylim = c(0, 54E3)) +
   scale_y_continuous(breaks = c(0, 20E3, 40E3), labels = c("0", "2", "4")) +
...
<<'figref-MB-ethanol-fitted', eval=FALSE, echo=FALSE>>=
df <- read.table(file = here("data/abscoeff-H2O+EtOH.dat"))
names(df) <- 
   c("wl", "abs", 
     "F1", "F2", "F3", "F4", "F5", "F6",
     "model", "residuals")
ggplot() + 
   ...
   geom_text(
      ...
      size = 2.0) +
   # solvent label
   annotate("text", x = 500, y = 54E3, label = "\\ch{H2O}:EtOH (90:10)", hjust = 0, vjust = 1, size = 2) +
   coord_cartesian(xlim = c(500, 750), ylim = c(0, 54E3)) +
   scale_y_continuous(breaks = c(0, 20E3, 40E3), labels = c("0", "2", "4")) +
   scale_x_continuous(breaks = seq(200, 1000, 100)) +
   labs(y = "$\\alpha/\\num[retain-unity-mantissa = false]{1e4}\\unit{\\litre\\per\\mole\\per\\cm}$",
...
<<'tabref-abscoeff-MB', eval=FALSE, echo=FALSE>>=
xtab.abscoeff <-
   # move the rownames into the df proper
   cbind(solvent = paste0("\\ch{", rownames(abs.coeff.df), "}"),
         abs.coeff.df) %>%
   xtable()
caption(xtab.abscoeff) <- 
   paste(
      "The absorption coefficients for the largest spectral peaks of methylene blue in the two studied solvents.",
      "The presence of ethanol does not affect the strongest peak, but clearly decreases the absorbance of the other peaks.",
      "Please note that the presented uncertainties are not derived from the multiple standard curves, but simply the result of smoothing the absorbance across a \\qty{", MB.bandwidth, "}{\\nm} band centered on each peak, illustrated in \\cref{fig:MB-abs-coefficient} (this was primarily done as a way to handle the noisy values of the largest peak).")
label(xtab.abscoeff) <- "tab:abscoeff-MB"
names(xtab.abscoeff) <- 
   c("{Solvent}",
     "{$\\lambda\\approx\\qty{612}{\\nm}$}",
     "{$\\lambda\\approx\\qty{629}{\\nm}$}",
     "{$\\lambda\\approx\\qty{665}{\\nm}$}")
digits(xtab.abscoeff) <- 
   c(0, #row.names
     0, #solvent
     5, #612
     5, #629
     5) #665
display(xtab.abscoeff) <- 
   c("s", #row.names
     "s", #solvent
     "f", #612
     "f", #629
...
print(
   ...
   add.to.row = 
      list(
         pos = 
            list(-1, 0, nrow(xtab.abscoeff)),
         command = 
            c(
               paste(
                  "\\toprule\n",
                  # top header goes here
                  " & \\multicolumn{3}{c}{$\\alpha(\\lambda)/\\unit{\\litre\\per\\mol\\per\\cm}$} \\\\\n",
                  "\\cmidrule(lr){2-4}"),
               "\\midrule\n", 
               "\\bottomrule\n")),
...
# I adjusted k1 and k2 to make the kinetic fit plot and table easier to interpret
# by setting k1 to the same value for all sizes, including the small diams which 
# lacked a "dip" in their CC0 plots
cutoffs <- 
   data.frame(
      structure(
         dimnames = 
...
spectrometer.resolution <- wavelength2energy(354) - wavelength2energy(355.5)
@
...
<<'read-N04H-small-nostir'>>=
pc.abs.small.N04H.nostir <- 
   LoadRData2Variable(here("data/abs-pc-spectra-N04H-small-nostir.rda.gz")) %>%
   filter(intensity >= 0) %>%
...
<<'NPBE-N04H-small-nostir-photodegradation-fit-bandgap'>>=
...
<<'NPBE-N04H-small-nostir-photodegradation-energy-ceiling'>>=
...
<<'NPBE-N04H-small-nostir-photodegradation-max-sqabs-ceiling-floor'>>=
...
<<'MB-N04H-small-nostir-fityk-script', eval=FALSE, echo=FALSE>>=
spectra <-
   data.frame(
      files = 
         list.files(
            path = here("data/MB-N04H-small-nostir"),
            pattern = "\\.txt$",
            full.names = TRUE))
...
fityk.session.file <- here("data/session-N04H-small.fit")
...
<<'MB-N04H-small-nostir-fityk-errors'>>=
fityk.files <- 
   list.files(
      path = here("data/MB-N04H-small-nostir"),
      pattern = "*.errors",
...
<<'MB-N04H-small-nostir-fityk-peaks'>>=
fityk.files <- 
   list.files(
      path = here("data/MB-N04H-small-nostir"),
      pattern = "*.peaks",
   ...
   fityk.peaks.small.N04H.nostir.MB.max <-
      rbind(
         ...
         cbind(
            spectra = 
               as.numeric(
                  sub("PC_N04H_small_nostir_", "", sub("\\.peaks", "", basename(fityk.files[i])))),
            read.table(
               fityk.files[i],
               skip = 1,
               # skip the last three columns (fityk's output is verbose)
...
<<'figref-MB-N04H-small-nostir-photodegradation', eval=FALSE>>=
nth.spectra <- 1
this.data <- 
   pc.abs.small.N04H.nostir %>%
...
# :: this plot includes every nth spectra (every spectra if n=1)
p <- 
   ggplot() +
   ### TAUC ANALYSIS
...
<<'NPBE-N02A-small-nostir-photodegradation-fit-bandgap'>>=
# the band edge eventually flattens out
...
<<'NPBE-N02A-small-nostir-photodegradation-energy-ceiling'>>=
# note, we only need one sampleid in ceiling (we are using the same ceiling for all spectra)
sqabs.ceiling <- 
   data.frame(eV = seq(max(E.range.ceiling), min(E.range.ceiling), 
...
<<'NPBE-N02A-small-nostir-photodegradation-max-sqabs-ceiling-floor'>>=
# upper and lower bounds for linear fit (squared abs, intersect with abscissa determines bandgap)
bg.fit.upper <- 0.7 # 0.7 * (ceiling - floor)
bg.fit.lower <- 0.2 # 0.2 * (ceiling - floor)
# make a condensed df (one row per sampleid, ie one row per spectrum)
...
<<'MB-N02A-small-nostir-fityk-script', eval=FALSE, echo=FALSE>>=
spectra <-
   data.frame(
      files = 
         list.files(
            path = here("data/OOHR2000/N02A-small"),
            pattern = "\\.txt$",
            full.names = TRUE))
spectra$number <- seq(0, length(spectra$files) - 1)
spectra$fityk.read <- paste0("@+ < '", spectra$files, "'")
...
# use it like this: cat(spectra$print.data, sep = "\n")
# Finally, you may want to save the complete Fityk session state (very large ASCII file)
fityk.session.file <- here("data/session-N02A-small.fit")
spectra$save.session <- 
   paste0(
      "info state > '",
      fityk.session.file,
...
<<'MB-N02A-small-nostir-fityk-errors'>>=
fityk.files <- 
   list.files(
      path = here("data/OOHR2000/N02A-small"),
      pattern = "*.errors",
...
<<'MB-N02A-small-nostir-fityk-peaks'>>=
...
# join height error from errors into this df 
fityk.peaks.small.N02A.nostir.MB.max <-
   left_join(fityk.peaks.small.N02A.nostir.MB.max, 
...
<<'read-N04H-medium-nostir'>>=
pc.abs.medium.N04H.nostir <- 
   LoadRData2Variable(here("data/abs-pc-spectra-N04H-medium-nostir.rda.gz")) %>%
   filter(intensity >= 0) %>%
   filter(wavelength >= 285 & wavelength <= 1050)
# add time and adjusted time to df
# time can be constructed by reading the five last digits of sampleid
...
<<'NPBE-N04H-medium-nostir-photodegradation-fit-bandgap'>>=
# the band edge eventually flattens out
...
<<'NPBE-N04H-medium-nostir-photodegradation-energy-ceiling'>>=
# note, we only need one sampleid in ceiling (we are using the same ceiling for all spectra)
sqabs.ceiling <- 
   data.frame(eV = seq(max(E.range.ceiling), min(E.range.ceiling), 
...
<<'NPBE-N04H-medium-nostir-photodegradation-max-sqabs-ceiling-floor', warning=FALSE>>=
# upper and lower bounds for linear fit (squared abs, intersect with abscissa determines bandgap)
bg.fit.upper <- 0.7 # 0.7 * (ceiling - floor)
bg.fit.lower <- 0.2 # 0.2 * (ceiling - floor)
# make a condensed df (one row per sampleid, ie one row per spectrum)
...
<<'MB-N04H-medium-nostir-fityk-script', eval=FALSE, echo=FALSE>>=
spectra <-
   data.frame(
      files = 
         list.files(
            path = here("data/MB-N04H-medium-nostir"),
            pattern = "\\.txt$",
            full.names = TRUE))
spectra$number <- seq(0, length(spectra$files) - 1)
spectra$fityk.read <- paste0("@+ < '", spectra$files, "'")
...
# use it like this: cat(spectra$print.data, sep = "\n")
# Finally, you may want to save the complete Fityk session state (very large ASCII file)
fityk.session.file <- here("data/session-N04H-medium.fit")
spectra$save.session <- 
   paste0(
      "info state > '",
      fityk.session.file,
...
<<'MB-N04H-medium-nostir-fityk-errors'>>=
fityk.files <- 
   list.files(
      path = here("data/MB-N04H-medium-nostir"),
      pattern = "*.errors",
...
for (i in 1:length(fityk.files)) {
   this.df <-
      ...
      mutate(spectra = basename(fityk.files[i]) %>% 
                sub(x=., "\\.errors", "") %>% 
                sub(x=., "PC_MB_N04H_medium_nostir_", "") %>% 
                as.numeric())
...
<<'MB-N04H-medium-nostir-fityk-peaks'>>=
# read the peak parameters for the MB main abs peak from fityk datafiles
fityk.files <-
   list.files(
      path = here("data/MB-N04H-medium-nostir"),
...
for (i in 1:length(fityk.files)) {
   fityk.peaks.medium.N04H.nostir.MB.max <-
      rbind(fityk.peaks.medium.N04H.nostir.MB.max,
            cbind(spectra = as.numeric(sub("PC_MB_N04H_medium_nostir_", "",
                                           sub("\\.peaks", "", basename(fityk.files[i])))),
                  read.table(fityk.files[i],
                             skip = 1,
                             # skip the last three columns (fityk's output is verbose)
...
<<'read-N02A-large-nostir'>>=
#### N02A # large nanoparticles (initially)
# remove the noisy ends and data points below the abscissa
pc.abs.large.N02A.nostir <- 
   LoadRData2Variable(here("data/abs-pc-spectra-N02A-large-nostir.rda.gz")) %>%
   filter(intensity >= 0) %>%
   filter(wavelength >= 285 & wavelength <= 1050)
# add time and adjusted time to df
# time can be constructed by reading the five last digits of sampleid
...
<<'NPBE-N02A-large-nostir-photodegradation-fit-bandgap'>>=
# the band edge eventually flattens out
...
<<'NPBE-N02A-large-nostir-photodegradation-energy-ceiling'>>=
# note, we only need one sampleid in ceiling (we are using the same ceiling for all spectra)
sqabs.ceiling <- 
   data.frame(eV = 
...
<<'NPBE-N02A-large-nostir-photodegradation-max-sqabs-ceiling-floor'>>=
# upper and lower bounds for linear fit (squared abs, intersect with abscissa determines bandgap)
...
<<'MB-N02A-large-nostir-fityk-script', eval=FALSE, echo=FALSE>>=
### ONLY RUN THIS BLOCK MANUALLY, WHEN NECESSARY
spectra <-
   data.frame(
      files = 
         list.files(
            path = here("data/OOHR2000/N02A-H02AD"),
            pattern = "*.txt",
            full.names = TRUE))
spectra$number <- seq(0, length(spectra$files) - 1)
spectra$fityk.read <- paste0("@+ < '", spectra$files, "'")
...
# use it like this: cat(spectra$print.data, sep = "\n")
# Finally, you may want to save the complete Fityk session state (very large ASCII file)
fityk.session.file <- here("data/session-N02A-large.fit")
spectra$save.session <- 
   paste0(
      "info state > '",
      fityk.session.file,
...
<<'MB-N02A-large-nostir-fityk-errors'>>=
fityk.files <- 
   list.files(
      path = here("data/OOHR2000/N02A-H02AD"),
      pattern = "*.errors",
...
<<'MB-N02A-large-nostir-fityk-peaks'>>=
# read the peak parameters for the MB main abs peak from fityk datafiles
fityk.files <-
   list.files(
      path = here("data/OOHR2000/N02A-H02AD"),
...
<<'read-N04E-large-nostir'>>=
pc.abs.large.N04E.nostir <- 
   LoadRData2Variable(here("data/abs-pc-spectra-N04E-large.rda.gz")) %>%
   filter(intensity >= 0) %>%
   filter(wavelength >= 285 & wavelength <= 1050)
# add time and adjusted time to df
# time can be constructed by reading the five last digits of sampleid
...
# express wavele.nngth as energy (convert nm to eV)
...
<<'NPBE-N04E-large-nostir-photodegradation-fit-bandgap'>>=
# the band edge eventually flattens out
...
<<'NPBE-N04E-large-nostir-photodegradation-energy-ceiling'>>=
# note, we only need one sampleid in ceiling (we are using the same ceiling for all spectra)
sqabs.ceiling <- 
   data.frame(eV = 
                 seq(max(E.range.ceiling), min(E.range.ceiling), 
...
<<'NPBE-N04E-large-nostir-photodegradation-max-sqabs-ceiling-floor'>>=
# upper and lower bounds for linear fit (squared abs, intersect with abscissa determines bandgap)
bg.fit.upper <- 0.7 # 0.7 * (ceiling - floor)
bg.fit.lower <- 0.2 # 0.2 * (ceiling - floor)
# make a condensed df (one row per sampleid, ie one row per spectrum)
...
<<'MB-N04E-large-nostir-fityk-script', eval=FALSE, echo=FALSE>>=
spectra <-
   data.frame(
      files = 
         list.files(
            path = here("data/MB-N04E-large-nostir"),
            pattern = "*.txt",
            full.names = TRUE))
spectra$number <- seq(0, length(spectra$files) - 1)
spectra$fityk.read <- paste0("@+ < '", spectra$files, "'")
...
# use it like this: cat(spectra$print.errors, sep = "\n")
# Finally, you may want to save the complete Fityk session state (very large ASCII file)
fityk.session.file <- here("data/session-N04E-large.fit")
spectra$save.session <- 
   paste0(
      "info state > '",
      fityk.session.file,
...
<<'MB-N04E-large-nostir-fityk-errors'>>=
fityk.files <- 
   list.files(
      path = here("data/MB-N04E-large-nostir"),
      pattern = "*.errors",
...
for (i in 1:length(fityk.files)) {
   this.df <-
      ...
      mutate(spectra = basename(fityk.files[i]) %>% 
                sub(x=., "\\.errors", "") %>% 
                sub(x=., "PC_MB_N04E_large_nostir_", "") %>% 
                as.numeric())
...
<<'MB-N04E-large-nostir-fityk-peaks'>>=
# read the peak parameters for the MB main abs peak from fityk datafiles
fityk.files <-
   list.files(
      path = here("data/MB-N04E-large-nostir"),
...
for (i in 1:length(fityk.files)) {
   fityk.peaks.large.N04E.nostir.MB.max <-
      rbind(fityk.peaks.large.N04E.nostir.MB.max,
            cbind(spectra = as.numeric(sub("PC_MB_N04E_large_nostir_", "",
                                           sub("\\.peaks", "", basename(fityk.files[i])))),
                  read.table(fityk.files[i],
                             skip = 1,
                             # skip the last three columns (fityk's output is verbose)
...
<<'figref-MB-N04E-large-nostir-photodegradation', eval=FALSE>>=
nth.spectra <- 1
this.data <- 
   pc.abs.large.N04E.nostir %>%
...
fityk.peaks.all <- 
   rbind(cbind(sample = "N02A-small-nostir",
               np.size = "small",
               stirring = "no",
               ...
               approx.init.np.diam = condensed.all %>% 
                  ...
                  unique() %>%
                  unlist(use.names = F),
               left_join(x = fityk.peaks.small.N02A.nostir.MB.max %>%
                            filter(peakno %% 2 == 1),
                         y = condensed.all %>% 
                            filter(sample == "N02A-small-nostir") %>%
                            select(spectra, fit.Eg, fit.np.diam, fit.np.area, fit.np.vol),
                         by = "spectra")),
         cbind(sample = "N04H-small-nostir",
               np.size = "small",
               stirring = "no",
               init.np.diam = condensed.all %>% 
               ...
               approx.init.np.diam = condensed.all %>% 
                  ...
                  unique() %>%
                  unlist(use.names = F),
               left_join(x = fityk.peaks.small.N04H.nostir.MB.max %>%
                            filter(peakno %% 2 == 1),
                         y = condensed.all %>% 
                            filter(sample == "N04H-small-nostir") %>%
                            select(spectra, fit.Eg, fit.np.diam, fit.np.area, fit.np.vol),
                         by = "spectra")),
         cbind(sample = "N04H-medium-nostir",
               np.size = "medium",
               stirring = "no",
               init.np.diam = condensed.all %>% 
               ...
               approx.init.np.diam = condensed.all %>% 
                  ...
                  unique() %>%
                  unlist(use.names = F),
               left_join(x = fityk.peaks.medium.N04H.nostir.MB.max %>%
                            filter(peakno %% 2 == 1),
                         y = condensed.all %>% 
                            filter(sample == "N04H-medium-nostir") %>%
                            select(spectra, fit.Eg, fit.np.diam, fit.np.area, fit.np.vol),
                         by = "spectra")),
         cbind(sample = "N02A-large-nostir",
               np.size = "large",
               stirring = "no",
               init.np.diam = condensed.all %>% 
               ...
               approx.init.np.diam = condensed.all %>% 
                  ...
                  unique() %>%
                  unlist(use.names = F),
               left_join(x = fityk.peaks.large.N02A.nostir.MB.max %>%
                            filter(peakno %% 2 == 1),
                         y = condensed.all %>% 
                            filter(sample == "N02A-large-nostir") %>%
                            select(spectra, fit.Eg, fit.np.diam, fit.np.area, fit.np.vol),
                         by = "spectra")),
         cbind(sample = "N04E-large-nostir",
               np.size = "large",
               stirring = "no",
               init.np.diam = condensed.all %>% 
               ...
               approx.init.np.diam = condensed.all %>% 
                  ...
                  unique() %>%
                  unlist(use.names = F),
               left_join(x = fityk.peaks.large.N04E.nostir.MB.max %>%
                            filter(peakno %% 2 == 1),
                         y = condensed.all %>% 
                            filter(sample == "N04E-large-nostir") %>%
                            select(spectra, fit.Eg, fit.np.diam, fit.np.area, fit.np.vol),
...
fityk.peaks.all$cumflux <- cumflux(energy2wavelength(fityk.peaks.all$fit.Eg))
# convert minutes (spectra) to seconds
fityk.peaks.all$seconds <- 60 * fityk.peaks.all$spectra
@


<<'number-ZnO-nanoparticles-model'>>=
...
# (we assume the number stays constant, aka nucleation and growth)
...
<<'average-NP-growth'>>=
# What is the average rate of growth in diameter for the different sizes?
growthrate <- 
   cbind(
      condensed.all %>% 
      filter(spectra <= 10) %>%
      # group by sample to get an lm for each line in the plot fig:NPBE-diameter-all
      # group_by(np.size, stirring, sample) %>%
...
sun.exp <- OO2df(here("data/AM15_solar_simulator.txt"))
...
<<'photon-flux-per-bandgap', cache=FALSE>>=
...
<<'tabref-growthrate', eval=FALSE, echo=FALSE>>=
this.tab <-
   growthrate %>%
...
align(xtab.growthrate) <- 
   c("l", #row.names
     "S[separate-uncertainty, table-format=1.2(1)]", #diameter
     "S[separate-uncertainty, table-format=1.3(1)]", #slope
     "S[separate-uncertainty, table-format=1.3(1)]", #intercept
...
print(xtab.growthrate,
      ...
      tabular.environment = "tabular",
      latex.environments = c("center", "small"),
      # size = "small" does not set the caption in small, which is what the RSC template wants
      # size = "small",
      sanitize.text.function = function(x){x},
      math.style.negative = FALSE)
@
...
<<'tabref-bandgaps', eval=FALSE, echo=FALSE>>=
xtab.bandgaps <-
   tribble(
...
<<'figref-solarlamp', eval=FALSE, echo=FALSE>>=
p <-
   ggplot() +
...
q <- 
   ...
   geom_line(aes(x = energy, y = cumflux),
             colour = "white") + 
...
<<'figref-MB-N02A-small-nostir-mbabs', eval=FALSE, echo=FALSE>>=
nth.spectra <- 1
this.data <- 
   pc.abs.small.N02A.nostir %>%
...
<<'figref-MB-N02A-small-nostir-edge', eval=FALSE, echo=FALSE>>=
nth.spectra <- 1
this.data <- 
   pc.abs.small.N02A.nostir %>%
   filter(time.abs %in% seq(0, length(unique(pc.abs.small.N02A.nostir$time.abs)), by = nth.spectra))
...
<<'figref-MB-N04H-medium-nostir-mbabs', eval=FALSE, echo=FALSE>>=
nth.spectra <- 1
this.data <- 
   pc.abs.medium.N04H.nostir %>%
...
<<'figref-MB-N04H-medium-nostir-edge', eval=FALSE, echo=FALSE>>=
nth.spectra <- 1
this.data <- 
   pc.abs.medium.N04H.nostir %>%
   filter(time.abs %in% seq(0, length(unique(pc.abs.medium.N04H.nostir$time.abs)), by = nth.spectra))
...
<<'figref-MB-N02A-large-nostir-mbabs', eval=FALSE, echo=FALSE>>=
nth.spectra <- 1
this.data <- 
   pc.abs.large.N02A.nostir %>%
...
<<'figref-MB-N02A-large-nostir-edge', eval=FALSE, echo=FALSE>>=
nth.spectra <- 1
this.data <- 
   pc.abs.large.N02A.nostir %>%
   filter(time.abs %in% seq(0, length(unique(pc.abs.large.N02A.nostir$time.abs)), by = nth.spectra))
...
<<'tabref-kinetic-parameters-MB', eval=FALSE, echo=FALSE>>=
...
<<'figref-MB-ratio-fityk-CC0', eval=FALSE, echo=FALSE>>=
slice.small <- 
  fityk.peaks.all %>%
...
<<'figref-MB-nlog-fityk-all', eval=FALSE, echo=FALSE>>=
...
<<'figref-NPBE-diameter-all', eval=FALSE, echo=FALSE>>=
...
<<'figref-growthrate-vs-initial-diameter', eval=FALSE, echo=FALSE>>=
this.fit <- lm(formula = slope ~ intercept + I(intercept^2), data = growthrate)
# # just to confirm that quadratic model actually fits the data
# growthrate$y <- # note the non-intuitive order of the lm coefficients in the formula
#    summary(this.fit)$coef[2] * growthrate$intercept + 
#    summary(this.fit)$coef[3] * growthrate$intercept^2 +
#    summary(this.fit)$coef[1]
# # ok, it does, I checked.
ggplot(data = growthrate %>% arrange(desc(intercept)),
       aes(y = slope, 
           x = intercept)) +
   geom_smooth(method = "lm", 
               formula = y ~ x + I(x^2),
               se = FALSE) +
   # error-bars in y-direction not shown
   # geom_errorbarh(aes(y = slopewerror,
   #                   xmin = errors_min(interceptwerror),
   #                   xmax = errors_max(interceptwerror)),
   #                height = 0.0010) +
   # slope and intercept from log-linear model
   geom_errorbar(aes(x = interceptwerror,
                     ymin = errors_min(slopewerror),
                     ymax = errors_max(slopewerror))) +
   geom_point(size = 0.9) +
   scale_y_continuous(breaks = seq(0, 0.08, 0.02)) +
   annotate(geom = "text", x = 6, y = 0.05, colour = "blue", size = 2.5,
            label = paste0("$", "\\Delta d = ", 
                           "\\num[separate-uncertainty=false]{", formatC(summary(this.fit)$coef[2,1], format="f", digits=2), 
                           "+-",
                           formatC(summary(this.fit)$coef[2,2], format="f", digits=2), "}",
                           "d_\\text{i}",
                           " + ",
                           "\\num[separate-uncertainty=false]{", formatC(summary(this.fit)$coef[3,1], format="f", digits=3), 
                           "+-",
                           formatC(summary(this.fit)$coef[3,2], format="f", digits=3), "}",
                           "d_\\text{i}^2",
                           " + ",
                           "\\num[separate-uncertainty=false]{", formatC(summary(this.fit)$coef[1,1], format="f", digits=2), 
                           "+-",
                           formatC(summary(this.fit)$coef[1,2], format="f", digits=2), "}",
                           "$")) +
   annotate(geom = "text", x = 6 - 0.5, y = 0.05, colour = "blue", size = 2.5,
            label = paste0("$R^2 = \\num{", 
